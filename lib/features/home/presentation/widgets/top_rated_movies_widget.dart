...
class TopRatedMoviesWidget extends StatelessWidget {
  ...
  const TopRatedMoviesWidget({super.key, required this.bloc , required this.title});
            ...
            if (state is LoadingGetMoviesState) {
              ...
              return Column(
                children: [
                  HeadSectionWidget(
                    voidCallback: (){
                      getAppRouter.push(ShowAllMoviesRoute(title: title, movies: state.topRatedMovies));
                    },
                    title: title,
                    buttonText: 'View All',
                  ),
